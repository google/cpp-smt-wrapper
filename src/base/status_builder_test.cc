...
// https://github.com/google/zetasql/blob/master/zetasql/base/status_builder_test.cc
...
const SourceLocation kLoc = SOURCE_LOC;
...
TEST(StatusBuilderTest, Ctors) {
  EXPECT_EQ(ToStatus(StatusBuilder(kZomg, SOURCE_LOC) << "zomg"),
            absl::Status(kZomg, "zomg"));
}

TEST(StatusBuilderTest, Identity) {
  SourceLocation loc(SOURCE_LOC);
...
TEST(CanonicalErrorsTest, CreateAndClassify) {
  for (const auto& test : kCanonicalErrorTests) {
    ...
    absl::Status status = test.creator(SOURCE_LOC) << message;
