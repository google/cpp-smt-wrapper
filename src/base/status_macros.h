...
// https://github.com/google/zetasql/blob/master/zetasql/base/status_macros.h

#ifndef SRC_BASE_STATUS_MACROS_H_
#define SRC_BASE_STATUS_MACROS_H_
...
//   }
//
// The macro ends with a `smt::base::StatusBuilder` which allows the returned
...
// to avoid confusion between a `smt::base::StatusBuilder` and a
...
#define RETURN_IF_ERROR(expr)                               \
  ...
  if (::smt::base::status_macro_internal::StatusAdaptorForMacros \
          status_macro_internal_adaptor = {(expr), SOURCE_LOC}) {  \
  } else /* NOLINT */                                               \
...
// well as a `smt::base::StatusBuilder` object populated with the error and
...
#define ASSIGN_OR_RETURN(...)                              \
   STATUS_MACROS_IMPL_GET_VARIADIC_(                        \
      __VA_ARGS__, STATUS_MACROS_IMPL_ASSIGN_OR_RETURN_3_, \
...
#define STATUS_MACROS_IMPL_ASSIGN_OR_RETURN_3_(lhs, rexpr,         \
                                                       error_expression)   \
...
#define STATUS_MACROS_IMPL_ASSIGN_OR_RETURN_(statusor, lhs, rexpr, \
  ...
  if (ABSL_PREDICT_FALSE(!statusor.ok())) {                                \
    ::smt::base::StatusBuilder _(std::move(statusor).status(),          \
                                    SOURCE_LOC);                          \
...
class StatusAdaptorForMacros {
 public:
  ...
  smt::base::StatusBuilder&& Consume() { return std::move(builder_); }

 private:
  smt::base::StatusBuilder builder_;
};

}  // namespace status_macro_internal
}  // namespace smt::base

#endif  // SRC_BASE_STATUS_MACROS_H_
