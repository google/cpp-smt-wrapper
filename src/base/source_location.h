...
// https://github.com/google/zetasql/blob/master/zetasql/base/source_location.h

#ifndef SRC_BASE_SOURCE_LOCATION_H_
#define SRC_BASE_SOURCE_LOCATION_H_
...
// callsite, define it with a parameter of type `smt::base::SourceLocation`.
// The caller can then invoke the function, passing `SOURCE_LOC` as the
// argument.
//
...
#define SMT_INTERNAL_HAS_KEYWORD(x) !(__is_identifier(x))
#else
#define SMT_INTERNAL_HAS_KEYWORD(x) 0
#endif

#if !defined(SMT_INTERNAL_HAVE_SOURCE_LOCATION_CURRENT)
#if SMT_INTERNAL_HAS_KEYWORD(__builtin_LINE) && \
    SMT_INTERNAL_HAS_KEYWORD(__builtin_FILE)
#define SMT_INTERNAL_HAVE_SOURCE_LOCATION_CURRENT 1
#else
#define SMT_INTERNAL_HAVE_SOURCE_LOCATION_CURRENT 0
#endif
#endif

#undef SMT_INTERNAL_HAS_KEYWORD

namespace smt::base {

// Class representing a specific location in the source code of a program.
// `smt::base::SourceLocation` is copyable.
class SourceLocation {
 ...
 public:
  ...
  // by the `SOURCE_LOC` macro, hence the name.
...
#if SMT_INTERNAL_HAVE_SOURCE_LOCATION_CURRENT
 ...
 private:
  // Do not invoke this constructor directly. Instead, use the `SOURCE_LOC`
  // macro below.
  //
  // `file_name` must outlive all copies of the `smt::base::SourceLocation`
...
// If a function takes an `smt::base::SourceLocation` parameter, pass this as
// the argument.
#define SOURCE_LOC \
  ::smt::base::SourceLocation::DoNotInvokeDirectly(__LINE__, __FILE__)

#endif  // SRC_BASE_SOURCE_LOCATION_H_
